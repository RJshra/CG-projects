...
void Transformation::handleInput() {
	...
	glm::vec3 temp0 = { -10.0f,0,0 };
	_positions[0] = asin(sin(total_time)) * velocity + temp0;
	_rotateAngles[1] = total_time * angulerVelocity;
	_scales[2] = asin(sin(total_time)) * scaleRate + _scales[0];
	// --------------------------------------------------
}
...
void Transformation::renderFrame() {
	...
	for (std::size_t i = 0; i < _bunnies.size(); ++i) {
		...
		rotation = glm::rotate(rotation, glm::radians(_rotateAngles[i] * (180.0f / PI)), _rotateAxis[i]);
		// @scale
