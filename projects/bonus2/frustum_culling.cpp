...
void FrustumCulling::initGPUCullingResources() {
	...
	glBufferData(GL_ARRAY_BUFFER, _amount * sizeof(int), nullptr, GL_STATIC_READ);
	...
	glBufferData(GL_ARRAY_BUFFER, _amount * sizeof(glm::mat4), &_modelMatrices[0], GL_DYNAMIC_DRAW);
	...
	////glGenBuffers(1, &_vbo);
	//glBindBuffer(GL_ARRAY_BUFFER, _instancedAsternoids->getInstacenVbo());
	//glBufferData(GL_ARRAY_BUFFER, _amount * sizeof(glm::mat4),_modelMatrices.data(), GL_DYNAMIC_DRAW);
...
void FrustumCulling::renderFrame() {
	...
	case Method::CPU:
		...
		_frustumCullingShader->setUniformVec3("boundingBox.min", box.min);
		_frustumCullingShader->setUniformVec3("boundingBox.max", box.max);
		_frustumCullingShader->setUniformVec3("frustum.planes[0].normal", frustum.planes[0].normal);
		_frustumCullingShader->setUniformFloat("frustum.planes[0].signedDistance", frustum.planes[0].signedDistance);
		_frustumCullingShader->setUniformVec3("frustum.planes[1].normal", frustum.planes[1].normal);
		_frustumCullingShader->setUniformFloat("frustum.planes[1].signedDistance", frustum.planes[1].signedDistance);
		_frustumCullingShader->setUniformVec3("frustum.planes[2].normal", frustum.planes[2].normal);
		_frustumCullingShader->setUniformFloat("frustum.planes[2].signedDistance", frustum.planes[2].signedDistance);
		_frustumCullingShader->setUniformVec3("frustum.planes[3].normal", frustum.planes[3].normal);
		_frustumCullingShader->setUniformFloat("frustum.planes[3].signedDistance", frustum.planes[3].signedDistance);
		_frustumCullingShader->setUniformVec3("frustum.planes[4].normal", frustum.planes[4].normal);
		_frustumCullingShader->setUniformFloat("frustum.planes[4].signedDistance", frustum.planes[4].signedDistance);
		_frustumCullingShader->setUniformVec3("frustum.planes[5].normal", frustum.planes[5].normal);
		_frustumCullingShader->setUniformFloat("frustum.planes[5].signedDistance", frustum.planes[5].signedDistance);
		...
		glGetBufferSubData(GL_TRANSFORM_FEEDBACK_BUFFER, 0, _amount * sizeof(int), _visibles.data());
