...
void TextureMapping::initBlendShader() {
	...
	const char* fsCode =
		...
		"	vec3 diffuse1 = light.intensity*light.color * max(dot(lightDir, fNormal), 0.0f) * material.kds[0];\n"
		"	vec3 diffuse2 = light.intensity*light.color * max(dot(lightDir, fNormal), 0.0f) * material.kds[1];\n"
		"	vec4 res0=vec4(texture(mapKds[0], fTexCoord).rgb*diffuse1,1.0);\n"
		"	vec4 res1=vec4(texture(mapKds[1], fTexCoord).rgb*diffuse2,1.0);\n"
		"	color = mix(res0,res1,material.blend);\n"
		"}\n";
	//----------------------------------------------------------------
...
void TextureMapping::initCheckerShader() {
	...
	const char* fsCode =
		...
		"void main() {\n"
		"	float temp = 1.0/material.repeat;\n"
		"	float x = fTexCoord[0];\n"
		"	while(x>=2*temp)x-=2*temp;\n"
		"	float y = fTexCoord[1];\n"
		"	while(y>=2*temp)y-=2*temp;\n"
		"	if((x>temp&&y<temp)||(x<temp&&y>temp))color = vec4(material.colors[0], 1.0f);\n"
		"	else color = vec4(material.colors[1], 1.0f);\n"
		"}\n";
	//----------------------------------------------------------------
...
void TextureMapping::renderFrame() {
	...
	case RenderMode::Blend:
		...
		glUniform1i(glGetUniformLocation(_blendShader->_handle, "mapKds[0]"), 0);
		glUniform1i(glGetUniformLocation(_blendShader->_handle, "mapKds[1]"), 1);
		//----------------------------------------------------------------
		break;
